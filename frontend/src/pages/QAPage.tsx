...
const QAPage: React.FC<QAPageProps> = ({isAuthenticated = false}) => {
    ...
    const checkAdminRole = async (u: UserData) => {
        try {
            const res = await fetch(
                `http://192.168.0.101:8000/api/user/role/?user_id=${u.id}`
            );
    ...
    const fetchQA = async () => {
        setLoading(true);
        try {
            const res = await fetch("http://192.168.0.101:8000/api/QA/");
    ...
    const handleAskQuestion = async () => {
        ...
        try {
            const res = await fetch("http://192.168.0.101:8000/api/QA/create/", {
                method: "POST",
    ...
    const handleDelete = async (qaId: number) => {
        ...
        try {
            const response = await fetch(`http://192.168.0.101:8000/api/QA/${qaId}/delete/`, {
                method: "DELETE",
                headers: { "Content-Type": "application/json" },
    ...
    const handleAnswer = async (qaId: number) => {
        ...
        try {
            ...
            const res = await fetch(`http://192.168.0.101:8000/api/QA/${qaId}/answer/`, {
                method: "PATCH",
                headers: {"Content-Type": "application/json"},
                body: JSON.stringify(payload),
            });
