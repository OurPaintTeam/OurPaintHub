...
const DownloadPage: React.FC<DownloadPageProps> = ({ isAuthenticated = false }) => {
  ...
  useEffect(() => {
    ...
    fetch("http://192.168.0.101:8000/api/download/")
      .then((res) => res.json())
      .then((data) => {
        setDownloads(data);
        setLoading(false);
  ...
  const handleDeleteVersion = async (versionId: number) => {
    ...
    try {
      const response = await fetch(`http://192.168.0.101:8000/api/download/${versionId}/delete/`, {
        method: "DELETE",
  ...
  const handleDownload = async (versionId: number, filename: string) => {
    ...
    try {
      ...
      const url = user_id 
        ? `http://192.168.0.101:8000/api/download/${versionId}/?user_id=${user_id}`
        : `http://192.168.0.101:8000/api/download/${versionId}/`;
