...
const ProjectsPage: React.FC = () => {
    ...
    const fetchUserProjects = async () => {
        ...
        try {
            const response = await fetch(`http://192.168.0.101:8000/api/project/get_user_projects/`,{
                method: "POST",
    ...
    const fetchFriends = async () => {
        if (!user) return alert("Сначала авторизуйтесь");
        try {
            const response = await fetch(`http://192.168.0.101:8000/api/friends/?user_id=${user.id}`);
            if (!response.ok) return;
            const data = await response.json();
            setFriends(data);
    ...
    const fetchReceivedProjects = async () => {
        if (!user) return alert("Сначала авторизуйтесь");
        try {
            const response = await fetch(`http://192.168.0.101:8000/api/project/shared/`,{
                method: "POST",
                headers: { "Content-Type": "application/json" },
    ...
    const handleUploadProject = async (file: File, projectName: string, description: string, isPrivate: boolean) => {
        ...
        try {
            const response = await fetch(`http://192.168.0.101:8000/api/project/add/`, {
                method: "POST",
                body: formData,
            });
            if (response.ok) {
    ...
    const sendProjectToFriend = async (userId:number,friendId: number, comment: string) => {
        ...
        try {
            const response = await fetch(`http://192.168.0.101:8000/api/project/share/${sharingProject.id}/`, {
                method: "POST",
                headers: {"Content-Type": "application/json"},
                body: JSON.stringify({
