...
const PublicAccountPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const resp = await fetch(`http://192.168.0.101:8000/api/profile/?user_id=${id}`);
        ...
        try {
          ...
          const friendsResp = await fetch(`http://192.168.0.101:8000/api/friends/?user_id=${parsed.id}`);
          if (friendsResp.ok) {
  ...
  const loadFriendFriends = async (friendId: number) => {
    ...
    try {
      const resp = await fetch(`http://192.168.0.101:8000/api/friends/?user_id=${friendId}`);
      if (resp.ok) {
        const data = await resp.json();
  ...
  const handleAddFriend = async () => {
    ...
    setAdding(true);
    try {
      const resp = await fetch(`http://192.168.0.101:8000/api/friends/add/`, {
        method: 'POST',
  ...
  const handleRemoveFriend = async () => {
    ...
    setRemoving(true);
    try {
      const resp = await fetch(`http://192.168.0.101:8000/api/friends/remove/`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ user_id: currentUserId, friend_id: Number(id) })
      });
