...
const FriendsPage: React.FC = () => {
    ...
    const loadFriends = async (userId: number, search: string = "") => {
        try {
            ...
            const response = await fetch(`https://localhost:8000/api/friends/?user_id=${userId}${searchParam}`);
            if (response.ok) {
    ...
    const loadAllUsers = async (userId: number, friendsList: UserData[] = friends, search: string = "") => {
        try {
            const searchParam = search ? `&search=${encodeURIComponent(search)}` : "";
            const response = await fetch(`https://localhost:8000/api/users/?exclude_id=${userId}${searchParam}`);
            if (response.ok) {
                const data = await response.json();
    ...
    const loadRequests = async (userId: number) => {
        try {
            const response = await fetch(`https://localhost:8000/api/friends/requests/?user_id=${userId}`);
            if (response.ok) {
                const data = await response.json();
                const enriched = await enrichUsersWithAvatar(data);
    ...
    const loadSentRequests = async (userId: number) => {
        try {
            const response = await fetch(`https://localhost:8000/api/friends/requests/sent/?user_id=${userId}`);
            if (response.ok) {
                const data = await response.json();
                const enriched = await enrichUsersWithAvatar(data);
    ...
    const handleRespond = async (fromUserId: number, action: 'accept' | 'decline') => {
        if (!user) return;
        try {
            const response = await fetch(`https://localhost:8000/api/friends/requests/respond/`, {
                method: 'POST',
    ...
    const handleAddFriend = async (friendId: number) => {
        ...
        try {
            const response = await fetch(`https://localhost:8000/api/friends/add/`, {
                method: "POST",
                headers: {"Content-Type": "application/json"},
                body: JSON.stringify({
                    user_id: user.id,
    ...
    const handleRemoveFriend = async (friendId: number, e: React.MouseEvent) => {
        ...
        try {
            const response = await fetch(`https://localhost:8000/api/friends/remove/`, {
                method: "DELETE",
                headers: {"Content-Type": "application/json"},
                body: JSON.stringify({
                    user_id: user.id,
    ...
    const handleCancelRequest = async (receiverId: number) => {
        ...
        try {
            const response = await fetch(`https://localhost:8000/api/friends/requests/cancel/`, {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
    ...
    const enrichUsersWithAvatar = async (users: UserData[]) => {
        return Promise.all(users.map(async (u) => {
            try {
                const res = await fetch(`https://localhost:8000/api/profile/?user_id=${u.id}`);
                if (res.ok) {
                                ...
                                <div key={req.id} className="user-card">
                                    ...
                                    <div style={{display: 'flex', gap: '0.5rem'}}>
                                        <button className="add-friend-btn"
                                                onClick={() => handleRespond(req.id, 'accept')}>Принять
                                        </button>
                                        <button className="add-friend-btn" style={{background: '#6b7280'}}
                                                onClick={() => handleRespond(req.id, 'decline')}>Отклонить
                                        </button>
                                    </div>
                                </div>
                            ))
