...
const AccountPage: React.FC = () => {
    ...
    const loadUserProfile = async (userId: number) => {
        try {
            // Профиль
            const profileResponse = await fetch(`https://localhost:8000/api/profile/?user_id=${userId}`);
            ...
            // Роль
            const roleResponse = await fetch(`https://localhost:8000/api/user/role/?user_id=${userId}`);
            if (roleResponse.ok) {
            ...
            // Мои проекты
            const projectsResponse = await fetch(`https://localhost:8000/api/project/get_user_projects/`,{
                method: "POST",
            ...
            // Полученные проекты
            const receivedResponse = await fetch(`https://localhost:8000/api/project/shared/`,{
                method: "POST",
                headers: {"Content-Type": "application/json"},
                body: JSON.stringify({
                    user_id: userId
            ...
            // Друзья
            const friendsResponse = await fetch(`https://localhost:8000/api/friends/?user_id=${userId}`);
