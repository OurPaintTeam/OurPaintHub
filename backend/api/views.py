...
def get_company(request, company_id):
    ...
    return Response(serialize_company(company, user), status=status.HTTP_200_OK)
...
def create_company(request):
    ...
    return Response({"message": "Компания создана", "company": serialize_company(company, user)}, status=status.HTTP_201_CREATED)
...
def update_company(request, company_id):
    ...
    return Response({"message": "Компания обновлена", "company": serialize_company(company, user)}, status=status.HTTP_200_OK)
...
@api_view(["GET"])
def get_company_repositories(request, company_id):
    user, error = get_user_from_request_data(request)
    if error:
        return error
    ...
    repositories = Repository.objects.filter(owner_company=company).order_by("name")
